...
function InitSelfInfo() {
    ...
    api.Post("/Api/User/GetSelfInfo", {}, (resData) => {
        userInfo = JSON.parse(resData);
        storage.SelfInfo = userInfo;
    });
}

function InitFocusUserList() {
    let focusUserList;
...
function InitMyWebSocket() {
    ...
    document.querySelector("#button-submit").onclick = (ele, ev) => {
        ...
        focusUserModelList.Change(focusUserModelList.GetLength() - 1, focusUserModel => focusUserModel.LastMessage = sendMessage.Content);
    }
    ...
    mywebsocket.AddEventHandler(MyWebSocket.WebSocketMessageEvent.OnServerMessage, async (wsMessage) => {
        ...
        focusUserModelList.Change(focusUserModelList.GetLength() - 1, focusUserModel => focusUserModel.LastMessage = resMessage.Content);
    });
}
