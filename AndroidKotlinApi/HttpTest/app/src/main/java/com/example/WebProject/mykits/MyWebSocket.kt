...
class MyWebSocket(url: String) {
    ...
    data class WebSocketMessageEvent(val Code: Int) {
        companion object {
            ...
            val OnClientTempMessage = WebSocketMessageEvent(400)
            val OnServerTempMessage = WebSocketMessageEvent(401)
            val OnDataSideTempMessage = WebSocketMessageEvent(402)
    ...
    data class WebSocketMessage(
        val MessageEvent: WebSocketMessageEvent,
...
///// <returns></returns>
    private fun SetAMessage(bytes: ByteArray, startIndex: Int, wsMessage: WebSocketMessage): Int {
        ...
        eventCodeSegment.order(ByteOrder.LITTLE_ENDIAN).putInt(wsMessage.MessageEvent.Code)
    ...
    fun SendMessages(wsMessages: ArrayList<WebSocketMessage>) {
        ...
        if(wsMessages == null || wsMessages.isEmpty()) return
        var totalLength = 0;
        wsMessages.forEach {
